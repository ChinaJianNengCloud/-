...
class Encoder(nn.Module):
    def __init__(self, input_dim, hidden_size,n_layers=1):
        ...
        self.gru = nn.GRU(input_dim, hidden_size, n_layers,
                           bidirectional=True)#GRU模型#input_dim特征维度#隐藏状态的维度
...
class Attention(nn.Module):
    ...
    def forward(self, hidden, encoder_outputs):
        ...
        attn_energies = torch.bmm(v, energy).squeeze(1)  # [B, T]#表示注意力的张量
        return F.softmax(attn_energies,dim=1).unsqueeze(1)#返回归一化之后注意力分布的张量
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, trg, teacher_forcing_ratio=0.5):
        ...
        for t in range(1, max_len):
            ...
            #output = Variable(trg.data[t] if is_teacher else top1).cuda()形如[max_len, batch_size, vocab_size]将2048变成vocab_size
